...
// Pesach words to find (English with Hebrew)
const PESACH_WORDS = [
  { english: 'PESACH', hebrew: 'פסח', found: false },
...
function generateGrid(): { grid: CellData[][]; placedWords: PlacedWord[] } {
  ...
  const sortedWords = [...PESACH_WORDS].sort((a, b) => b.english.length - a.english.length);
      ...
      >
        ...
        <Animated.View entering={FadeIn.duration(400).delay(400)}>
          ...
          <Text style={{ color: '#64748b', textAlign: 'center', marginTop: 8, fontSize: 16 }}>
            You found all {totalWords} Pesach words!
          </Text>
        </Animated.View>
...
export default function PesachWordSearchGame() {
  const isWeb = Platform.OS === 'web';
  ...
  // Word colors for found words (pink-themed for Pesach)
  const wordColors = [
    '#fde047', // Yellow
    '#86efac', // Green
    '#c4b5fd', // Violet
