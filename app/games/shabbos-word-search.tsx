...
// Shabbos words to find (English with Hebrew)
const SHABBOS_WORDS = [
  { english: 'SHABBOS', hebrew: 'שבת', found: false },
  { english: 'CHALLAH', hebrew: 'חלה', found: false },
  { english: 'KIDDUSH', hebrew: 'קידוש', found: false },
  { english: 'HAVDALAH', hebrew: 'הבדלה', found: false },
  { english: 'CANDLES', hebrew: 'נרות', found: false },
  { english: 'SEUDAH', hebrew: 'סעודה', found: false },
  { english: 'ZEMIROS', hebrew: 'זמירות', found: false },
  { english: 'ONEG', hebrew: 'עונג', found: false },
];
...
function generateGrid(): { grid: CellData[][]; placedWords: PlacedWord[] } {
  ...
  const sortedWords = [...SHABBOS_WORDS].sort((a, b) => b.english.length - a.english.length);
      ...
      >
        ...
        <Animated.View entering={FadeIn.duration(400).delay(400)}>
          ...
          <Text style={{ color: '#64748b', textAlign: 'center', marginTop: 8, fontSize: 16 }}>
            You found all {totalWords} Shabbos words!
          </Text>
        </Animated.View>
        ...
        <Animated.View entering={FadeIn.duration(400).delay(500)} className="mt-8">
          <View className="items-center bg-violet-50 rounded-2xl py-6 px-8">
            <Text className="text-5xl font-bold text-violet-600">{formatTime(elapsedTime)}</Text>
...
export default function ShabbosWordSearchGame() {
  ...
  // Word colors for found words (purple-themed for Shabbos)
  const wordColors = [
    '#c4b5fd', // Violet
    '#86efac', // Green
    '#fde047', // Yellow
