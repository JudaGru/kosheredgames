...
function Cell({ correctLetter, userLetter, isSelected, isHighlighted, cellNumber, onPress, cellSize, isCorrect, showAnswer }: { row: number; col: number; correctLetter: string | null; userLetter: string; isSelected: boolean; isHighlighted: boolean; cellNumber: number | null; onPress: () => void; cellSize: number; isCorrect: boolean; showAnswer: boolean; }) {
  ...
  const getBgColor = () => { if (isCorrect && userLetter) return '#dcfce7'; if (isSelected) return '#dbeafe'; if (isHighlighted) return '#eff6ff'; return 'white'; };
  const getBorderColor = () => { if (isSelected) return '#2563eb'; if (isHighlighted) return '#60a5fa'; return '#cbd5e1'; };
...
function ClueItem({ clue, isSelected, isCompleted, onPress }: { clue: ClueData; isSelected: boolean; isCompleted: boolean; onPress: () => void; }) {
  return (
    <Pressable onPress={onPress} style={{ flexDirection: 'row', paddingVertical: 10, paddingHorizontal: 12, backgroundColor: isSelected ? '#eff6ff' : isCompleted ? '#dcfce7' : 'white', borderRadius: 8, marginBottom: 6, borderWidth: isSelected ? 2 : 1, borderColor: isSelected ? '#2563eb' : isCompleted ? '#86efac' : '#e2e8f0' }}>
      <Text style={{ fontSize: 14, fontWeight: '700', color: '#2563eb', width: 24 }}>{clue.number}.</Text>
...
export default function HanukkahCrosswordGame() {
  const isWeb = Platform.OS === 'web';
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#f8fafc' }}>
      ...
      <View style={{ backgroundColor: 'white', borderBottomWidth: 1, borderBottomColor: '#e2e8f0' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 12 }}>
          ...
          <Text style={{ fontWeight: 'bold', color: '#1e293b', fontSize: isWeb ? 20 : 18 }}>Hanukkah Crossword</Text>
