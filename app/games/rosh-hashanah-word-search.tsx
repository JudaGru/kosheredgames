...
// Rosh Hashanah words to find (English with Hebrew)
const ROSH_HASHANAH_WORDS = [
  { english: 'SHOFAR', hebrew: 'שופר', found: false },
  { english: 'TESHUVAH', hebrew: 'תשובה', found: false },
  { english: 'APPLES', hebrew: 'תפוחים', found: false },
  { english: 'HONEY', hebrew: 'דבש', found: false },
  { english: 'PRAYER', hebrew: 'תפילה', found: false },
  { english: 'MACHZOR', hebrew: 'מחזור', found: false },
  { english: 'POMEGRANATE', hebrew: 'רימון', found: false },
  { english: 'TEKIAH', hebrew: 'תקיעה', found: false },
];
...
function generateGrid(): { grid: CellData[][]; placedWords: PlacedWord[] } {
  ...
  const sortedWords = [...ROSH_HASHANAH_WORDS].sort((a, b) => b.english.length - a.english.length);
...
function HeaderButton({
  ...
  const animatedStyle = useAnimatedStyle(() => ({
    ...
    backgroundColor: interpolate(bgColor.value, [0, 1], [0, 1]) === 1
      ? (variant === 'refresh' ? '#fef3c7' : '#e2e8f0')
      : (variant === 'refresh' ? '#fef3c7' : '#f1f5f9'),
  }));
    ...
    <Pressable onPress={handlePress} onHoverIn={handleHoverIn} onHoverOut={handleHoverOut}>
      ...
      >
        <FontAwesome
          name={icon as any}
          size={18}
          color={variant === 'refresh' ? '#d97706' : '#64748b'}
        />
      </Animated.View>
    </Pressable>
  );
...
function Cell({ cell, isSelected, isFoundWord, foundWordColor, onMouseDown, onMouseEnter, onPress, cellSize, index, gameKey }: CellProps) {
  ...
  useEffect(() => {
    ...
    entranceAnim.value = withDelay(
      index * 5, // Faster animation for larger grid
      ...
      >
        ...
        <Animated.View entering={FadeIn.duration(400).delay(400)}>
          ...
          <Text style={{ color: '#64748b', textAlign: 'center', marginTop: 8, fontSize: 16 }}>
            You found all {totalWords} Rosh Hashanah words!
          </Text>
        </Animated.View>
        ...
        <Animated.View entering={FadeIn.duration(400).delay(500)} className="mt-8">
          <View className="items-center bg-amber-50 rounded-2xl py-6 px-8">
            <Text className="text-5xl font-bold text-amber-600">{formatTime(elapsedTime)}</Text>
            <Text className="text-sm text-amber-700 mt-2 uppercase tracking-wide font-semibold">
              Time
            </Text>
          </View>
        </Animated.View>
...
export default function RoshHashanahWordSearchGame() {
  const isWeb = Platform.OS === 'web';
  ...
  // Word colors for found words (amber-themed for Rosh Hashanah)
  const wordColors = [
    '#fde047', // Yellow
    '#86efac', // Green
    '#c4b5fd', // Violet
  ...
  const calculateCellSize = useCallback(() => {
    ...
    const maxCellWidth = (availableWidth - GRID_SIZE * 3) / GRID_SIZE;
    const maxCellHeight = (availableHeight - GRID_SIZE * 3) / GRID_SIZE;

    return Math.min(maxCellWidth, maxCellHeight, useMobileLayout ? 28 : 40);
  ...
  // Calculate cell size including margin (1.5px on each side for 12x12 grid)
  const cellWithMargin = cellSize + 3;
