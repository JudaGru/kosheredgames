...
function Cell({ correctLetter, userLetter, isSelected, isHighlighted, cellNumber, onPress, cellSize, isCorrect, showAnswer }: { row: number; col: number; correctLetter: string | null; userLetter: string; isSelected: boolean; isHighlighted: boolean; cellNumber: number | null; onPress: () => void; cellSize: number; isCorrect: boolean; showAnswer: boolean; }) {
  ...
  const getBgColor = () => { if (isCorrect && userLetter) return '#dcfce7'; if (isSelected) return '#fef3c7'; if (isHighlighted) return '#fefce8'; return 'white'; };
  const getBorderColor = () => { if (isSelected) return '#d97706'; if (isHighlighted) return '#fbbf24'; return '#cbd5e1'; };
...
function ClueItem({ clue, isSelected, isCompleted, onPress }: { clue: ClueData; isSelected: boolean; isCompleted: boolean; onPress: () => void; }) {
  return (
    <Pressable onPress={onPress} style={{ flexDirection: 'row', paddingVertical: 10, paddingHorizontal: 12, backgroundColor: isSelected ? '#fefce8' : isCompleted ? '#dcfce7' : 'white', borderRadius: 8, marginBottom: 6, borderWidth: isSelected ? 2 : 1, borderColor: isSelected ? '#d97706' : isCompleted ? '#86efac' : '#e2e8f0' }}>
      <Text style={{ fontSize: 14, fontWeight: '700', color: '#d97706', width: 24 }}>{clue.number}.</Text>
  ...
  return (
    <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.75)', justifyContent: 'center', alignItems: 'center', padding: 20 }}>
      ...
      <Animated.View entering={FadeIn.duration(400).delay(200)} style={{ backgroundColor: 'white', borderRadius: 24, alignItems: 'center', width: '100%', maxWidth: 400, padding: isWeb ? 48 : 40 }}>
        ...
        <Text style={{ fontWeight: 'bold', color: '#1e293b', marginTop: 24, fontSize: isWeb ? 36 : 28 }}>Shanah Tovah!</Text>
        <Text style={{ color: '#64748b', textAlign: 'center', marginTop: 12, fontSize: 16 }}>You completed the Rosh Hashanah crossword!</Text>
        <Pressable onPress={onPlayAgain} style={{ backgroundColor: '#d97706', borderRadius: 16, paddingVertical: 16, paddingHorizontal: 32, marginTop: 32, width: '100%' }}><Text style={{ color: 'white', fontWeight: 'bold', fontSize: 18, textAlign: 'center' }}>Play Again</Text></Pressable>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#f8fafc' }}>
      ...
      <View style={{ backgroundColor: 'white', borderBottomWidth: 1, borderBottomColor: '#e2e8f0' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 12 }}>
          ...
          <Text style={{ fontWeight: 'bold', color: '#1e293b', fontSize: isWeb ? 20 : 18 }}>Rosh Hashanah Crossword</Text>
