...
const HANUKKAH_WORDS = [
  ...
  { english: 'LATKES', hebrew: 'לביבות', found: false },
  { english: 'CANDLES', hebrew: 'נרות', found: false },
  { english: 'MIRACLE', hebrew: 'נס', found: false },
  { english: 'OIL', hebrew: 'שמן', found: false },
  { english: 'LIGHT', hebrew: 'אור', found: false },
  { english: 'GELT', hebrew: 'דמי חנוכה', found: false },
];
...
function generateGrid(): { grid: CellData[][]; placedWords: PlacedWord[] } {
  ...
  const sortedWords = [...HANUKKAH_WORDS].sort((a, b) => b.english.length - a.english.length);
...
function HeaderButton({
  ...
  const animatedStyle = useAnimatedStyle(() => ({
    ...
    backgroundColor: interpolate(bgColor.value, [0, 1], [0, 1]) === 1
      ? (variant === 'refresh' ? '#bae6fd' : '#e2e8f0')
      : (variant === 'refresh' ? '#e0f2fe' : '#f1f5f9'),
  }));
    ...
    <Pressable onPress={handlePress} onHoverIn={handleHoverIn} onHoverOut={handleHoverOut}>
      ...
      >
        <FontAwesome
          name={icon as any}
          size={18}
          color={variant === 'refresh' ? '#0369a1' : '#64748b'}
        />
      </Animated.View>
    </Pressable>
  );
    ...
    <Animated.View style={containerStyle}>
      <Pressable
        ...
        style={{
          ...
          borderColor: isSelected ? '#0369a1' : '#e2e8f0',
          shadowColor: '#000',
    ...
    >
      ...
      >
        ...
        <Animated.View entering={FadeIn.duration(400).delay(500)} className="mt-8">
          <View className="items-center bg-sky-50 rounded-2xl py-6 px-8">
            <Text className="text-5xl font-bold text-sky-600">{formatTime(elapsedTime)}</Text>
            <Text className="text-sm text-sky-700 mt-2 uppercase tracking-wide font-semibold">
              Time
            </Text>
          </View>
        </Animated.View>
      ...
      <View className="bg-white border-b border-slate-200">
        ...
        >
          ...
          <View className="items-center">
            ...
            <Text className={`font-bold text-sky-600 ${useMobileLayout ? 'text-xl' : 'text-2xl'} mt-1`}>
